...
export async function upsertTokenLimit(input: UpsertTokenLimitInput): Promise<TokenLimit> {
  ...
  await db.run(
    ...
    id, input.apiKeyId, scopeType, scopeValue, tokenLimit, resetInterval, resetTime, enabled
  );

  const row = await db.get(
    "SELECT * FROM api_key_token_limits WHERE api_key_id = ? AND scope_type = ? AND scope_value = ?",
    input.apiKeyId, scopeType, scopeValue
  );
    ...
    `SELECT * FROM api_key_token_limits
     WHERE api_key_id = ?
       ...
       AND (
         ...
         OR (scope_type = 'provider' AND scope_value = ?)
       )`,
    apiKeyId, model || "", provider || ""
  );
  return rows.map(rowToTokenLimit);
}
...
export async function deleteTokenLimit(id: string): Promise<boolean> {
  ...
  await db.run("DELETE FROM api_key_token_counters WHERE limit_id = ?", id);
  await db.run("DELETE FROM api_key_token_limit_reset_logs WHERE limit_id = ?", id);
  const info = await db.run("DELETE FROM api_key_token_limits WHERE id = ?", id);
...
export async function logTokenLimitReset(
  ...
  await db.run(
    `INSERT INTO api_key_token_limit_reset_logs (limit_id, reset_at, prev_tokens, window_start)
     VALUES (?, datetime('now'), ?, ?)`,
    limitId, Math.max(0, Math.floor(toNumber(prevTokens))), windowStart
  );
}
