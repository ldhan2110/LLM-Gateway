...
export async function getProviderMetrics(): Promise<ProviderMetricRow[]> {
  ...
  return db.all<ProviderMetricRow>(
    `SELECT
        c.provider,
...
export async function getSearchProviderStats(): Promise<SearchProviderStatRow[]> {
  const db = getDbClient();
  return db.all<SearchProviderStatRow>(
    `
...
export async function getSearchAggregateStats(todayIso: string): Promise<SearchAggregateStats> {
  const db = getDbClient();
  const row = await db.get<SearchAggregateStats>(
    `SELECT
        COUNT(*) as total,
...
export async function getSearchProviderCounts(): Promise<SearchProviderCountRow[]> {
  const db = getDbClient();
  return db.all<SearchProviderCountRow>(
 ...
 *
 * @param whereClause - SQL WHERE clause (may be empty string) using positional ?
 *                      params aligned with the `params` array.
 * @param params      - Positional params array (string values).
 */
export async function getFallbackStats(
  ...
  const db = getDbClient();
  const row = await db.get<FallbackStatsRow>(
    `
      SELECT
