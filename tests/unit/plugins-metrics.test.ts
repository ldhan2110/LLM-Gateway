...
test("recordPluginMetric stores call count", async () => {
  ...
  await recordPluginMetric("test-plugin", "onRequest", 3.1, false);
...
test("recordPluginMetric tracks errors", async () => {
  const { recordPluginMetric, getPluginMetrics } = await import("../../src/lib/db/pluginMetrics.ts");
  await recordPluginMetric("err-plugin", "onRequest", 1.0, true);

  const metrics = await getPluginMetrics("err-plugin");
  const m = metrics.find((r: { event: string }) => r.event === "onRequest");
  assert.ok(m, "should have onRequest metric");
...
test("recordPluginMetric tracks latency", async () => {
  const { recordPluginMetric, getPluginMetrics } = await import("../../src/lib/db/pluginMetrics.ts");
  await recordPluginMetric("latency-plugin", "onRequest", 42.5, false);

  const metrics = await getPluginMetrics("latency-plugin");
  const m = metrics.find((r: { event: string }) => r.event === "onRequest");
  assert.ok(m, "should have onRequest metric");
...
test("getPluginMetrics returns all plugins when no filter", async () => {
  const { recordPluginMetric, getPluginMetrics } = await import("../../src/lib/db/pluginMetrics.ts");
  await recordPluginMetric("p1", "onRequest", 1, false);
  await recordPluginMetric("p2", "onResponse", 2, false);
...
test("clearPluginMetrics removes metrics", async () => {
  ...
  await recordPluginMetric("clear-test", "onRequest", 1, false);
  await clearPluginMetrics("clear-test");

  const metrics = await getPluginMetrics("clear-test");
  const m = metrics.find((r: { event: string }) => r.event === "onRequest");
