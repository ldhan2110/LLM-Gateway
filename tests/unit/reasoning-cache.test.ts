...
describe("Reasoning Replay Cache — Service Layer", () => {
  ...
  it("should store and retrieve reasoning by tool_call_id", async () => {
    ...
    assert.equal((await getReasoningCache("call_test_1"))?.reasoning, "The user wants to read the file...");
  });
  ...
  it("should store arbitrary reasoning cache keys", async () => {
    ...
    assert.equal((await getReasoningCache("request:req_direct:message:1"))?.reasoning, "Keyed plan");
  });
  ...
  it("should list persisted entries for the dashboard API", async () => {
    ...
    const deepseekEntries = (await getReasoningCacheServiceEntries({ provider: "deepseek" })) as Array<{
      toolCallId: string;
      expiresAt: string;
    }>;
  ...
  it("should not return expired SQLite entries and cleanup should prune them", async () => {
    await clearReasoningCacheAll();
    await setReasoningCache("call_expired", "deepseek", "deepseek-chat", "Expired reasoning", -1_000);
